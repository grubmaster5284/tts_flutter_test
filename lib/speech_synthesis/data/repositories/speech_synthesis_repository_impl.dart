...
class SpeechSynthesisRepositoryImpl implements ISpeechSynthesisRepository {
  ...
  SpeechSynthesisRepositoryImpl(this._serviceFactory, this._localService);
  ...
  Future<Result<SpeechResponseModel, SpeechSynthesisError>> convertTextToSpeech(
    ...
    if (cachedResponse != null) {
      ...
      return Success(_dtoToDomain(cachedResponse));
    }
    ...
    if (formattedLanguage != null &&
        request.service == TTSServiceModel.gemini) {
  ...
  Future<String?> _saveAudioToFile(SpeechResponseDto dto) async {
    try {
      AppLogger.debug(
        ...
        tag: 'TTSRepository',
        data: {'format': dto.audioFormat},
      );

      // On web, we can't use path_provider, so skip file saving
      if (kIsWeb) {
        AppLogger.debug(
          'Skipping file save on web platform',
          tag: 'TTSRepository',
        );
        return null;
      }
      ...
      AppLogger.info(
        'Audio saved to file',
        tag: 'TTSRepository',
        data: {'path': filePath, 'size': audioBytes.length},
      );
  ...
  /// Converts a SpeechResponseDto to a SpeechResponseModel (domain entity)
  ///
  /// This is a manual conversion that ensures the extension method issues
  /// don't cause runtime errors. It manually constructs the domain model
  /// from the DTO fields.
  SpeechResponseModel _dtoToDomain(SpeechResponseDto dto) {
    return SpeechResponseModel(
      audioData: dto.audioData,
      audioFormat: AudioFormatVO(dto.audioFormat),
      durationMs: dto.durationMs,
      metadata: dto.metadata,
    );
  }
